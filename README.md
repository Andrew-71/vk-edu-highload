...
Не используем Anycast для нестатических данных, так как сессии длинные (ожидание в очереди может длиться час). При изменении маршрута по BGP (что редко, но случается) соединение оборвется. GeoDNS также дает нам возможность перераспеределять трафик по загруженности датацентров -- а BGP о загрузке ничего не знает. Health check-и и короткий TTL позволяют перераспределить трафик в течении минуты после аварии. Для статики можно использовать Anycast -- нам все равно, на какой сервер прийдет запрос.
