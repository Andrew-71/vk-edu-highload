...
Всего у нас приблизительно 56 млн запросов в день. Если считать, что среднее действие пользователя в конечном итоге превращается в чуть более 4–5 API запросов, получаем приблизительно 250 млн запросов в день. Это дает нам 2900 RPS и, при соотношении 3x пика к среднему значению за день, пик в 8700 RPS; ввиду истории высоких пиков на сайте при продаже билетов на ожидаемые события, целесообразно округлить до 10000 RPS.

| Метрика              | Значение     |
| -------------------- | ------------ |
| RPS                  | 2900         |
| RPS (пик в день)     | 10000        |

Если считать размер среднего запроса ~30 КБ, то получим 0.64 Gbit/s при нормальном RPS, максимальный поток в 2 Gbit/s, и ежесуточный объем данных в 7500 GB.
