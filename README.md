...
|-------------------------|----------------------------------------------------------------|------------------|----------------|
| Пользователи            | ~3 КБ на пользователя при 60 млн всего                         | ~170 GB          | Незначительный |
| Метаданные событий      | ~10 КБ на событие при ~1 млн всего                             | ~10 GB           | Незначительный |
| Данные сидячих событий  | 200 байт на ~10 тыс. сидений при ~100 тыс. сидячих событиях    | ~200 GB          | С событиями    |
| Билеты                  | ~1 КБ на билет при 646 млн. билетов в год и хранением на 5 лет | ~3.1 ТБ          | 620 GB/год     |
| Отзывы                  | ~1 КБ на отзыв при ~50 млн в год и хранением на 5 лет          | ~250 GB          | ~50 GB/год     |
| Медиа файлы событий     | 500 КБ на файл при 5 изображениях при ~1 млн событий           | ~2.5 TB          | С событиями    |
| Индекс поиска           | ~50% от метаданных событий                                     | ~5 GB            | С событиями    |
| Сессии, места в очереди | —                                                              | >128 GB          | Временное      |
| Логи                    | 250 млн запросов/день при ~500 байт на запрос                  | ~125 GB/день     | Десятки TB/год |
...
Если считать размер среднего запроса ~30 КБ, то получим 0.64 Gbit/s при нормальном RPS, максимальный поток в 53 Gbit/s, и ежесуточный объем данных в 7500 GB.
